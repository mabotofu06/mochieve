...
export const TemplatesWorkGroup = (props: Props) => {
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        ...
        const response = await getFetch<UserInfo>(API_INFO.ENDPOINT.CACHE_USER_INFO);
        ...
        if (response.status === 200) {
          ...
          logger.info(`クライアントサイド認証チェック結果: ${authResult ? '作成者' : '非作成者'}`, { userId: userInfo.id, workGroupOwnerId: props.workGroup.userInfo.id });
        } else {
          // 認証失敗の場合は非作成者として扱う
          setIsAuthor(false);
          logger.info("認証なし、または認証失敗のため非作成者として表示");
        }
      } catch (error) {
  ...
  return (
    <div className={`flex flex-col relative w-full h-screen items-center${props.workGroup.isClose ? ' bg-green-50' : ''}`}>
      <OrganismsPostListHeaderCard
        id={props.workGroup.id}
        userInfo={props.workGroup.userInfo}
        isAuthor={isAuthor}
        isClose={props.workGroup.isClose}
      ...
      {isAuthor &&
          <button
            className="bg-green-600 text-white py-4 px-6 rounded-4xl text-xl opacity-100 w-fit my-5 disabled:opacity-30"
            onClick={addPostForm}
            disabled={props.workGroup.isClose || checkTodayPosted()}
          >
            今日の進捗を投稿
          </button>
      }
